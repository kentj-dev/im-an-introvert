...
function parseStats(raw: unknown): UsageStats {
  ...
  return {
    date,
    hiddenCount:
      typeof source.hiddenCount === "number"
        ? Math.max(0, source.hiddenCount)
        : 0,
    activeSeconds:
      typeof source.activeSeconds === "number"
        ? Math.max(0, source.activeSeconds)
        : 0,
  };
}
...
export async function addUsage(delta: {
  ...
  const next: UsageStats = {
    date: today(),
    hiddenCount:
      current.hiddenCount + Math.max(0, Math.trunc(delta.hidden ?? 0)),
    activeSeconds:
      current.activeSeconds + Math.max(0, Math.trunc(delta.seconds ?? 0)),
  };
