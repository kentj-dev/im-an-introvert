...
export function NavCard({
  ...
  const inner = (
    <>
      {image ? (
        ...
        <span
          className={cn(
            ...
            tone === 'site' ? 'bg-white/70 text-tint-site-foreground' : 'bg-tint-site text-tint-site-foreground',
          )}
        >
      ...
      <span className="min-w-0 flex-1 text-left">
        <span className="block truncate text-[14px] leading-tight font-medium">{title}</span>
        {description ? (
          <span className="mt-0.5 block truncate text-[12px] text-muted-foreground">{description}</span>
        ) : null}
      </span>
  ...
  const shell = cn(
    ...
    onClick && 'cursor-pointer focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none',
    className,
  );
