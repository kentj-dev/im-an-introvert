...
export function ChatDetailView({ chat, globals, onPatch, onRemove }: ChatDetailViewProps) {
  ...
  const rows = (keys: typeof MESSENGER_CALL_ROWS, lockWhenFieldHidden: boolean) =>
    keys.map((row) => {
      const global = globals[row.key];
      const fieldHidden =
        lockWhenFieldHidden && row.key !== 'hideChatField' && (chat.hideChatField || globals.hideChatField);
      ...
      return (
        <SettingRow
          ...
          disabledHint={
            global ? 'Hidden in every chat by the global setting.' : 'The whole chat field is already hidden.'
          }
  ...
  return (
    <div className="space-y-3.5">
      <div className="flex items-start gap-2 px-0.5">
        ...
        <Badge variant="secondary" className="mt-0.5 bg-tint-success! text-tint-success-foreground!">
          <ShieldCheck />
          Protected
        </Badge>
      </div>

      <SettingSection title="Calls and group actions" description="Stops a misclick from ringing everyone in the chat.">
        {rows(MESSENGER_CALL_ROWS, false)}
      </SettingSection>

      <SettingSection title="Chat field" description="Hiding the field leaves reading, scrolling and media intact.">
      ...
      <SettingSection title="This chat">
        <div className="space-y-2 px-3 py-3">
          <div className="space-y-1.5">
            <Label htmlFor="chat-name" className="text-[12px] text-muted-foreground">
              Display name
            </Label>
            <Input
              id="chat-name"
            ...
            <p className="text-[11px] leading-snug text-muted-foreground">
              Just a label for this popup. The conversation ID below is the real identifier.
            </p>
          </div>
          <p className="truncate font-mono text-[11px] text-muted-foreground" title={chat.id}>
            ID {chat.id}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5">
          {confirmRemove ? (
            <>
              <p className="flex-1 text-[12.5px]">Remove protection from this chat?</p>
