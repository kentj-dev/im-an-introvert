...
export function QuickStats({ stats }: { stats: UsageStats }) {
  return (
    <section className="space-y-1.5">
      <h2 className="px-0.5 text-[15px] leading-tight font-semibold tracking-tight">Quick Stats</h2>
      <div className="flex gap-2.5">
        <StatCard icon={EyeOff} value={stats.hiddenCount.toLocaleString()} label="Elements hidden today" />
        <StatCard icon={Leaf} value={formatDuration(stats.activeSeconds)} label="Distraction-free browsing" />
      </div>
    </section>
  );
}
